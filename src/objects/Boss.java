...
public class Boss {
   ...
   private PlayerStudent player;
   ImageIcon icBoss = new ImageIcon("images/bossSizeup.png");
   ...
   public Boss(PlayerStudent player, int x, int y) {
      this.player = player;
      this.x = x;
      this.y = y;
      this.life=100;
   ...
   private void bulletCreate() {
      if (bCount % 200 == 0) {
         if(firePase = false) {
            ...
            for (int j = 1; j <= 5; j++) {
               way = 180 + (30 * j);
               bossAttack = new BossAttack(boss, player, x + 50, y + 650, way, 0.5, 0, 0); // 총알이 생성되는                                                                                 // 위치
               bossAttackList.add(bossAttack); // arrayList에 저장한다
            }

            for (int j = 1; j <= 5; j++) {
               way = 180 + (30 * j);
               bossAttack = new BossAttack(boss, player, x + 450, y + 350, way, 0.5, 0, 0); // 총알이 생성되는 위치
               bossAttackList.add(bossAttack); // arrayList에 저장한다
            }
         }
         if(firePase = true) {
            ...
            for (int j = 1; j <= 7; j++) {
               way = 180 + (25 * j);
               bossAttack = new BossAttack(boss, player, x + 50, y + 350, way, 0.5, 0, 0); // 총알이 생성되는                                                                                 // 위치
               bossAttackList.add(bossAttack); // arrayList에 저장한다
            }
      ...
      if (bCount % 600 == 0) {
         for (int j = 1; j <= 7; j++) {
            way = 180 + (25 * j);
            bossAttack = new BossAttack(boss, player, x + 250, y + 80, way, 1, 0, 0); // 총알이 생성되는                                                                                 // 위치
            bossAttackList.add(bossAttack); // arrayList에 저장한다
         }
      }
   }
